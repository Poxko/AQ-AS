...
/* Verifica se un codice è già utilizzato nell'array */
static int codiceEsistente(const Richiesta *richieste, int numeroRichieste, int codice)
{
    return cercaRichiestaPerCodice(richieste, numeroRichieste, codice) != -1;
}
                       ...
                       float costoStimato, const char *dataApertura)
{
    ...
    codice = generaNuovoCodice(richieste, *numeroRichieste);
    if (codiceEsistente(richieste, *numeroRichieste, codice)) {
        return -1;
    }
                        ...
                        int codice, float costoFinale)
{
    ...
    if (!costoFinaleValido(costoFinale)) {
        return -1;
    }
    ...
    if (richieste[indice].stato != COMPLETATA) {
        return -1;
    }
