...
static float calcolaCostoMedioFinale(const Richiesta *richieste, int numeroRichieste)
{
    ...
    return somma / (float)contatore;
}

/* Conta le richieste per ogni stato */
static void contaPerStato(const Richiesta *richieste, int numeroRichieste,
                          int conteggi[NUM_STATI])
{
    int i;
...
static int aggiungiTipoDispositivo(char tipi[][MAX_DISPOSITIVO], int *numeroTipi,
